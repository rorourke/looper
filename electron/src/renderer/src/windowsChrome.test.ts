...
const settingsUrl = new URL("./settingsWindow.css", import.meta.url);
const stylesUrl = new URL("./styles.css", import.meta.url);

test("places native Windows chrome above the Looper header", async () => {
  const [app, main, rendererMain, settings, styles] = await Promise.all([
  ...
  assert.match(
    settings,
    /:root\[data-platform="win32"\] \.settings-window-titlebar\s*\{\s*display: none;/
  );
